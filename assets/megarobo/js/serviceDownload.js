...
function getRobotList(page){
	$.ajax({
		url: "/api/download/getList?language=zh",
		type: "get",
		dataType: "json",
		data:{
			"page":page,
		...
		success: function(data){
			if(!data["code"]){
				$("#robotList>ul").html('<div style="padding:20px; text-align:center;">暂无数据</div>');
				$("#robotPage").hide();
				return false;
			}
			...
			for(var i =0;i<l ;i++){
				...
				lArr.push('<a class="lr" href="'+dataList[i]["annexfile"]+'"><img src="/assets/megarobo/images/down_btn_icon.png" />下载</a>');
				lArr.push('</div>');
				lArr.push('</div>');
				lArr.push('</li>');
			}
